...
if(isset($_POST["btn_submit_register"])){
    ...
    if(isset($_POST["email"])){
        ...
        if(!empty($email)){
            ...
            if(!preg_match($reg_email, $email)){
                $message = "<p class='message_error'><strong>Ошибка!</strong>Введен неправильный адрес почты</p>";
                redirect_to($message, 'form_register.php');
            }
            ...
            $query_select_user = $mysqli->query("SELECT `id` FROM `users` WHERE `email` = '".$email."'");
					...
					if(!$query_select_user){
						...
						$message = "<p class='message_error'><strong>Ошибка!</strong> Ошибка в запросе к Базе Данных, при проверке существования пользователя с таким адресом электронной почты. </p><p>Описание ошибки: $mysqli->error <br /> Код ошибки: $mysqli->errno </p>";
						redirect_to($message, 'form_register.php');
					}
            ...
            $message="<p class='message_error'><strong>Ошибка!</strong>Укажите Вашу почту</p>";
            redirect_to($message,'form_register.php');
        }
    }else{
        $message="<p class='message_error'><strong>Ошибка!</strong>Отсутствует поле для ввода почты</p>";
            redirect_to($message,'form_register.php');
    }
    ...
    if(isset($_POST["password"])){
        ...
        if(!empty($password)){
            ...
            $message="<p class='message_error'><strong>Ошибка!</strong>Укажите пароль</p>";
            redirect_to($message,'form_register.php');
        }
    }else{
    ...
    if(!$result_query_insert){
        ...
        $message ="<p class='success_message'>Регистрация прошла успешно!<br /> Теперь вы можете авторизироваться, исползьуя Вашу почту и пароль.</p>";
    ...
    exit("<p><strong>Ошибка!</strong>Вы зашли напрямую, поэтому нет данных для обработки.");
}
